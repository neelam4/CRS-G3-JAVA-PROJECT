...
public class StudentController {
	...
	@RequestMapping(produces=MediaType.APPLICATION_JSON, method=RequestMethod.PUT, value="/{studentId}/registerforCourse/{courseId}")
	@ResponseBody
	public ResponseEntity registerforCourse(@PathVariable int studentId,@PathVariable int courseId)throws Exception {
		if(null== student.registerforCourse(studentId,courseId)) {
			return new ResponseEntity("Already registered", HttpStatus.NOT_FOUND);
		}
		return new ResponseEntity(student.registerforCourse(studentId,courseId), HttpStatus.OK);
